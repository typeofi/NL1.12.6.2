...
public class MainMenuOptionsActivity extends PreferenceActivity implements
    ...
    private final Set<Switch> hasInflatedSwitches = new HashSet<Switch>();
    ...
    private Preference nerdy_stuff_page;
    ...
    protected Thread ui = new Thread(new Runnable() {
        private WeakReference<MainMenuOptionsActivity> activity = null;
    ...
    protected void setUp() {
        ...
        if (scriptsPreference != null) {
            ...
            try {
                if (!getPackageManager().getPackageInfo("net.zhuoweizhang.mcpelauncher.pro"
                                , 0)
                        .versionName.startsWith(MainActivity.SCRIPT_SUPPORT_VERSION))
                    scriptsPreference.setEnabled(false);
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        nerdy_stuff_page = findPreference("nerdy_stuff_page");
        if (nerdy_stuff_page != null) {
            nerdy_stuff_page.setOnPreferenceClickListener(this);
        }
        ...
        Log.i("BlockLauncher", "Has recorder: " + hasRecorder);
    ...
    private void controllerChange(Switch sw) {
        ...
        sw.setChecked(false);
        Utils.Default().edit().putBoolean("zz_use_controller", false).apply();
    ...
    private void startStuff() {
        startActivity(new Intent(this, NerdyStuffActivity.class));
    }
