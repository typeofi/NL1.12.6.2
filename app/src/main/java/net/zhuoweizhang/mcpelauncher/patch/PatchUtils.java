...
public final class PatchUtils {
    ...
    public static void copy(File from, File to) throws IOException {
        InputStream is = Files.newInputStream(from.toPath());
        int length = (int) from.length();
        byte[] data = new byte[length];
        is.read(data);
        is.close();
        OutputStream os = Files.newOutputStream(to.toPath());
        os.write(data);
        os.close();
    }
    ...
    public static byte[] createMovwInstr(int rd, int imm) {
        ...
        Log.i("BlockLauncher", "Port patch: " + Long.toString(instr, 16));
        return finalByte;
    }

    public static byte[] intToLEByteArray(long value) {
        return new byte[]{
                (byte) (value >>> 16),
                (byte) (value >>> 24),
                (byte) value,
