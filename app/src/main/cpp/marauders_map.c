...
void
MSSetAddressTranslationFunction(void *(*translationFunction)(void *), const char *tempFilePattern);
...
int marauder_remap_text(uintptr_t originalBegin, size_t length, const char *sharedPath) {
    ...
    void *reMapping = mmap(oldMapping, length, PROT_READ | PROT_EXEC, MAP_SHARED | MAP_FIXED,
                           remapped_text_fd, 0);
