...
void *bl_early_grab_static_method_run(void *arg) {
    ...
    jmethodID mid = (*env)->GetStaticMethodID(env, methodArg->javaClass, methodArg->methodName,
                                              methodArg->signature);
    return (void *) mid;
}
...
void *bl_early_grab_method_run(void *arg) {
    ...
    JNIEnv *env = bl_early_attach_jnienv();
    // hopefully there's tail call optimization
    jmethodID mid = (*env)->GetMethodID(env, methodArg->javaClass, methodArg->methodName,
                                        methodArg->signature);
    return (void *) mid;
}

JNIEXPORT jobject JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeGrabMethod
        (JNIEnv *env, jclass clazz, jclass targetClass, jstring methodNameString,
         jstring methodSigString, jboolean isStatic) {
    ...
    int error = pthread_create(&thread, NULL,
                               (isStatic ? bl_early_grab_static_method_run
                                         : bl_early_grab_method_run),
                               (void *) &methodArg);
    if (error) {
