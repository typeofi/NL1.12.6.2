...
static void populate_vtable_indexes(void *mcpelibhandle) {
    ...
    vtable_indexes.blockitem_get_level_data_for_aux_value = bl_vtableIndex(mcpelibhandle,
                                                                           "_ZTV9BlockItem",
    ...
    vtable_indexes.appplatform_get_screen_type = bl_vtableIndex(mcpelibhandle,
                                                                "_ZTV21AppPlatform_android23",
                                                                "_ZNK19AppPlatform_android13getScreenTypeEv");
    vtable_indexes.appplatform_get_edition = bl_vtableIndex(mcpelibhandle,
                                                            "_ZTV21AppPlatform_android23",
                                                            "_ZNK11AppPlatform10getEditionEv");
    vtable_indexes.appplatform_use_centered_gui = bl_vtableIndex(mcpelibhandle,
                                                                 "_ZTV21AppPlatform_android23",
...
static void
(*bl_MinecraftClient_startLocalServer)(MinecraftClient *, std::string const &, std::string const &,
                                       void *);
...
static TextureUVCoordinateSet
(*bl_Tile_getTextureUVCoordinateSet)(Tile *, std::string const &, int);

static Recipes *(*bl_Recipes_getInstance)();

static void (*bl_Recipes_addShapedRecipe)(Recipes *, std::vector<ItemInstance> const &,
...
static void
(*bl_BlockSource_fireBlockEvent_real)(BlockSource *source, int x, int y, int z, int type, int data);

static AABB *(*bl_Block_getAABB)(Block *, BlockSource &, BlockPos const &, AABB &, int, bool, int);

static AABB *
(*bl_ReedBlock_getAABB)(Block *, BlockSource &, BlockPos const &, AABB &, int, bool, int);
...
static void
(*bl_ItemEntity_ItemEntity)(Entity *, TileSource &, Vec3 const &, ItemInstance const &, int);
...
static void *
(*bl_SkeletonRenderer_render_real)(MobRenderer *, Entity &, Vec3 const &, float, float);
...
static void (*bl_LiquidBlockStatic_LiquidBlockStatic)
        (Block *, std::string const &, int, BlockID, void *, std::string const &,
         std::string const &);
...
static void dumpEnchantNames() {
    __android_log_print(ANDROID_LOG_INFO, "BlockLauncher", "enchant dump: %d",
                        Enchant::mEnchants.size());
    for (Enchant *enchant: Enchant::mEnchants) {
        __android_log_print(ANDROID_LOG_INFO, "BlockLauncher", "enchant %d %s", enchant->id,
                            enchant->description.c_str());
    }
}
...
void bl_ChatScreen_sendChatMessage_hook(void *chatScreen) {
    std::string *chatMessagePtr = (std::string *) ((uintptr_t) chatScreen +
                                                   CHATSCREEN_TEXTBOX_TEXT_OFFSET);
    ...
    jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "chatCallback",
                                           "(Ljava/lang/String;)V");
...
void bl_RakNetInstance_connect_hook(RakNetInstance *rakNetInstance, char const *host, int port) {
    ...
    //Call back across JNI into the ScriptManager
    jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "rakNetConnectCallback",
...
AABB *
bl_CustomBlock_getAABBHook(Block *block, BlockSource &blockSource, BlockPos const &pos, AABB &aabb,
                           int int1,
                           bool bool1, int int2) {
    int blockId = block->id;
    if (bl_custom_block_collisionDisabled[blockId])
        return bl_ReedBlock_getAABB(
                block, blockSource, pos, aabb, int1, bool1, int2);
...
AABB &
bl_CustomBlock_getVisualShape_hook(Tile *tile, unsigned char data, AABB &aabb, bool someBool) {
    if (data == 0) return bl_Tile_getVisualShape(tile, data, aabb, someBool);
    int blockId = tile->id;
    AABB **aabbs = bl_custom_block_visualShapes[blockId];
    AABB *aabbout;
    if (aabbs == nullptr || (aabbout = aabbs[data - 1]) == nullptr)
        return bl_Tile_getVisualShape(
                tile, data, aabb, someBool);
    return *aabbout;
}
...
void bl_CustomBlock_onRedstoneUpdate_hook(Block *block, BlockSource &source, BlockPos const &pos,
                                          int newLevel,
                                          bool something) {
    JNIEnv *env;
    int attachStatus = bl_JavaVM->GetEnv((void **) &env, JNI_VERSION_1_2);
    ...
    //Call back across JNI into the ScriptManager
    jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "redstoneUpdateCallback",
                                           "(IIIIZII)V");

    env->CallStaticVoidMethod(bl_scriptmanager_class, mid, pos.x, pos.y, pos.z, newLevel, something,
                              blockId,
                              blockData);
...
void
bl_ClientNetworkHandler_handleTextPacket_hook(void *handler, void *ipaddress, TextPacket *packet) {
    if (!(packet->type == 0 || packet->type == 1)) {
        // text or client message
        // just pass it along
    ...
    if (packet->type == 0) {
        ...
        preventDefaultStatus = false;
        //Call back across JNI into the ScriptManager
        jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "handleChatPacketCallback",
                                               "(Ljava/lang/String;)V");

        env->CallStaticVoidMethod(bl_scriptmanager_class, mid, messageJString);
    } else if (packet->type == 1) {
        if (packet->username.length() == 0 &&
            packet->message == "\xc2" "\xa7" "0BlockLauncher, enable scripts") {
            bl_onLockDown = false;
        }
        ...
        preventDefaultStatus = false;
        //Call back across JNI into the ScriptManager
        jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class,
                                               "handleMessagePacketCallback",
...
static void
bl_Level_explode_hook(Level *level, TileSource *tileSource, Entity *entity, Vec3 const &p,
                      float power,
                      bool onFire) {
    JNIEnv *env;
    //This hook can be triggered by ModPE scripts, so don't attach/detach when already executing in Java thread
...
static void
bl_BlockSource_fireBlockEvent_hook(BlockSource *source, int x, int y, int z, int type, int data) {
    JNIEnv *env;
    //This hook can be triggered by ModPE scripts, so don't attach/detach when already executing in Java thread
    int attachStatus = bl_JavaVM->GetEnv((void **) &env, JNI_VERSION_1_2);
    if (attachStatus == JNI_EDETACHED) {
    ...
    //Call back across JNI into the ScriptManager
    jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "blockEventCallback",
                                           "(IIIII)V");
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeSelectLevel
    ...
    bl_MinecraftClient_startLocalServer(bl_minecraft, worlddirstr, std::string(nameUtfChars),
                                        &levelSettings);
    env->ReleaseStringUTFChars(worlddir, utfChars);
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativePlaySound
        (JNIEnv *env, jclass clazz, jfloat x, jfloat y, jfloat z, jstring sound, jfloat volume,
         jfloat pitch) {
...
JNIEXPORT jstring JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeGetSignText
    ...
    SignBlockEntity *te = static_cast<SignBlockEntity *>(bl_localplayer->getRegion()->getBlockEntity(
            x, y, z));
...
void bl_changeEntitySkin(Entity *entity, const char *newSkin) {
    bl_mobTexturesMap[entity->getUniqueID()] = mce::TexturePtr(bl_minecraft->getTextures(),
                                                               newSkin);
}
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeSetSneaking
    ...
    void (*setSneaking)(Entity *, bool) = (void (*)(Entity *,
                                                    bool)) entity->vtable[vtable_indexes.mob_set_sneaking];
...
Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeGetTextureCoordinatesForItem
...
JNIEXPORT jboolean JNICALL
Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeGetTextureCoordinatesForBlock
        (JNIEnv *env, jclass clazz, jint itemId, jint itemDamage, jint side,
...
void bl_initCustomBlockVtable() {
    ...
    memcpy(bl_CustomLiquidBlockStatic_vtable, bl_LiquidBlockStatic_vtable,
           vtable_indexes.tile_vtable_size);
    ...
    memcpy(bl_CustomLiquidBlockDynamic_vtable, bl_LiquidBlockDynamic_vtable,
           vtable_indexes.tile_vtable_size);
...
void bl_buildTextureArray(TextureUVCoordinateSet *output[], std::string textureNames[],
                          int textureCoords[]) {
...
Tile *bl_createBlock(int blockId, std::string textureNames[], int textureCoords[], int materialType,
                     bool opaque,
                     int renderShape, const char *name, int customBlockType) {
    ...
    if (customBlockType == 0) {
        ...
        bl_LiquidBlockDynamic_LiquidBlockDynamic(retval, nameStr, blockId,
                                                 bl_getMaterial(materialType),
        ...
        bl_LiquidBlockStatic_LiquidBlockStatic(retval, nameStr, blockId, blockId - 1,
                                               bl_getMaterial(materialType),
                                               textureNames[0], textureNames[1]);
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeDefineBlock
    ...
    Tile *tile = bl_createBlock(blockId, myStringArray, myIntArray, materialBlockId, opaque,
                                renderType, utfChars,
                                customBlockType);
    env->ReleaseStringUTFChars(name, utfChars);
}
...
Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeBlockSetExplosionResistance
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeBlockSetShape
        (JNIEnv *env, jclass clazz, jint blockId, jfloat v1, jfloat v2, jfloat v3, jfloat v4,
         jfloat v5, jfloat v6,
         jint damage) {
    if (blockId < 0 || blockId > 255) return;
    Tile *tile = bl_Block_mBlocks[blockId];
...
bool bl_tryRemoveExistingRecipe(Recipes *recipeMgr, int itemId, int itemCount, int itemDamage,
                                int ingredients[],
    ...
    for (int i = recipesSize - 1; i >= 0; i--) {
        //TODO: inefficient?
        Recipe *recipe = (*recipesList)[i];
        if ((((uintptr_t) recipe->vtable) & ~0xf) !=
            (((uintptr_t) bl_ShapelessRecipe_vtable) & ~0xf)) {
...
bool bl_lookForExistingRecipe(Recipes *recipeMgr, int itemId, int itemCount, int itemDamage,
                              int ingredients[],
                              int ingredientsCount) {
    std::vector<Recipe *> *recipesList = &recipeMgr->recipes;
    int recipesSize = recipesList->size();
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeAddShapedRecipe
        (JNIEnv *env, jclass clazz, jint itemId, jint itemCount, jint itemDamage,
         jobjectArray shape,
         jintArray ingredientsArray) {
...
JNIEXPORT jboolean JNICALL
Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeEntityHasCustomSkin
        (JNIEnv *env, jclass clazz, jlong entityId) {
    Entity *entity = bl_getEntityWrapper(bl_level, entityId);
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeLevelAddParticle
        (JNIEnv *env, jclass clazz, jint type, jfloat x, jfloat y, jfloat z, jfloat xVel,
         jfloat yVel, jfloat zVel, jint data) {
    Vec3 pos{x, y, z};
...
JNIEXPORT void JNICALL
Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeDefinePlaceholderBlocks
        (JNIEnv *env, jclass clazz) {
#if 0
#ifdef __arm__
...
Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativePlayerGetPointedEntity
        (JNIEnv *env, jclass clazz) {
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeLevelSetGrassColor
    ...
    pos.y = 64;
    pos.z = z;
    bl_localplayer->getRegion()->setGrassColor(color, pos,
                                               3); //if you recall, 3 = full block update
}
...
JNIEXPORT void JNICALL
Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeBlockSetCollisionEnabled
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeSpawnerSetEntityType
    ...
    if (bl_level == NULL) return;
    MobSpawnerBlockEntity *te = static_cast<MobSpawnerBlockEntity *>(bl_localplayer->getRegion()->
...
JNIEXPORT jlong JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeDropItem
        (JNIEnv *env, jclass clazz, jfloat x, jfloat y, jfloat z, jfloat range, jint id, jint count,
         jint damage) {
    ItemInstance instance(id, count, damage);

    Entity *entity = (Entity *) ::operator new(kItemEntitySize);
    bl_ItemEntity_ItemEntity(entity, *(bl_localplayer->getRegion()), Vec3(x, y + range, z),
                             instance,
                             10 /* pickup delay */);
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeMobAddEffect
        (JNIEnv *env, jclass clazz, jlong entityId, jint id, jint duration, jint amplifier,
         jboolean ambient,
         jboolean showParticles) {
    Entity *entity = bl_getEntityWrapper(bl_level, entityId);
    if (entity == NULL) return;
...
JNIEXPORT jint JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeGetItemEntityItem
    ...
    ItemInstance *instance = (ItemInstance *) (((uintptr_t) entity) +
                                               kItemEntity_itemInstance_offset);
    switch (type) {
        case ITEMID:
...
static bool isLocalAddress(JNIEnv *env, jstring hostJString) {
    jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "isLocalAddress",
                                           "(Ljava/lang/String;)Z");
...
Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeLevelGetLightningLevel
        (JNIEnv *env, jclass clazz) {
    if (!bl_localplayer) return 0;
...
static void *
bl_SkeletonRenderer_render_hook(MobRenderer *renderer, Entity &ent, Vec3 const &vec, float a,
                                float b) {
    auto foundIter = bl_mobTexturesMap.find(ent.getUniqueID());
    if (foundIter != bl_mobTexturesMap.end()) {
...
JNIEXPORT void JNICALL
Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeBlockSetRedstoneConsumer
...
static void *
(*bl_Throwable_throwableHit_real)(Entity *entity, HitResult const &hitResult, int param1,
                                  int param2);

void *
bl_Throwable_throwableHit_hook(Entity *entity, HitResult const &hitResult, int param1, int param2) {
    JNIEnv *env;
    int attachStatus = bl_JavaVM->GetEnv((void **) &env, JNI_VERSION_1_2);
    if (attachStatus == JNI_EDETACHED) {
        bl_JavaVM->AttachCurrentThread(&env, NULL);
    ...
    //Call back across JNI into the ScriptManager
    jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "throwableHitCallback",
                                           "(JIIIIIFFFJ)V");
    ...
    env->CallStaticVoidMethod(bl_scriptmanager_class, mid, entity->getUniqueID(),
                              ...
                              hitResult.type == HIT_RESULT_ENTITY
                              ? (jlong) hitResult.entity->getUniqueID()
                              : (jlong) 0);
...
JNIEXPORT jboolean Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativePlayerEnchant
    ...
    Inventory *inventory = *((Inventory **) (((uintptr_t) bl_localplayer) +
...
JNIEXPORT void Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeLevelSetDifficulty
    ...
    if (bl_level == nullptr) return;
    void (*setDifficulty)(Level *, int) = (void (*)(Level *, int)) bl_level->vtable[
            vtable_indexes.level_set_difficulty - 2];
...
JNIEXPORT void Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeModPESetDesktopGui
    ...
    bl_AppPlatform_vtable[vtable_indexes.appplatform_get_screen_type] = desktop
                                                                        ? (void *) &
                    bl_AppPlatform_getScreenType_hook
                                                                        : bl_AppPlatform_getScreenType_real;
    bl_AppPlatform_vtable[vtable_indexes.appplatform_get_edition] = desktop
                                                                    ? (void *) &bl_AppPlatform_getEdition_hook
                                                                    : bl_AppPlatform_getEdition_real;
    bl_AppPlatform_vtable[vtable_indexes.appplatform_use_centered_gui] = desktop
                                                                         ? (void *) &
                    bl_AppPlatform_useCenteredGui_hook
                                                                         : bl_AppPlatform_useCenteredGui_real;
}
...
void bl_Player_addExperience_hook(Player *player, int experience) {
    ...
    //Call back across JNI into the ScriptManager
    jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "playerAddExperienceCallback",
                                           "(JI)V");
...
void bl_Player_addLevels_hook(Player *player, int experience) {
    ...
    //Call back across JNI into the ScriptManager
    jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "playerAddLevelsCallback",
                                           "(JI)V");
    env->CallStaticVoidMethod(bl_scriptmanager_class, mid, playerId, experience);
...
void bl_prepatch_cppside(void *mcpelibhandle_) {
    ...
    Elf32_Sym *itemsSym = (Elf32_Sym *) bl_marauder_translation_function(
            (void *) dobby_elfsym((void *) mcpelibhandle, "_ZN4Item6mItemsE"));
    // since value + base = addr:
    Elf32_Addr oldValue = itemsSym->st_value;
    itemsSym->st_value = ((uintptr_t) &bl_items) - mcpelibhandle->base;
    Elf32_Word oldSize = itemsSym->st_size;
    ...
    if (newItemsAddress != &bl_items) {
        ...
        __android_log_print(ANDROID_LOG_ERROR, "BlockLauncher",
                            "Failed to expand item array: expected %p got %p original %p",
    ...
    if (!bl_patch_got(mcpelibhandle, originalItemsAddress, (void *) &bl_items)) {
        ...
        itemsSym->st_size = oldSize;
        __android_log_print(ANDROID_LOG_ERROR, "BlockLauncher",
                            "Failed to expand item array: can't find got");
        return;
    }
    ...
    if (setItemCode[4] == 0x00 && setItemCode[5] == 0x7f) {
        ...
        __android_log_print(ANDROID_LOG_ERROR, "BlockLauncher",
                            "Failed to expand item array: can't patch setItem");
        return;
    }
#endif
#ifdef __i386
    ...
    mcpelauncher_hook((void *) &ItemRenderer::getGraphics,
...
void bl_setuphooks_cppside(soinfo2 *mcpelibhandle) {

//    soinfo2 *mcpelibhandle = (soinfo2 *) dlopen("libminecraftpe.so", RTLD_LAZY);XXX

    bl_Gui_displayClientMessage = (void (*)(void *, const std::string &)) dlsym(
            RTLD_DEFAULT, "_ZN3Gui20displayClientMessageERKSs");

    void *sendChatMessage = dlsym(RTLD_DEFAULT, "_ZN10ChatScreen15sendChatMessageEv");
    mcpelauncher_hook(sendChatMessage, (void *) &bl_ChatScreen_sendChatMessage_hook,
                      (void **) &bl_ChatScreen_sendChatMessage_real);

    bl_Item_Item = (void (*)(Item *, std::string const &, short)) dlsym(RTLD_DEFAULT,
                                                                        "_ZN4ItemC1ERKSss");

    // FIXME 0.11
    bl_MinecraftClient_startLocalServer = (void (
    *)(MinecraftClient *, std::string const &, std::string const &, void *))
    ...
    bl_MinecraftClient_leaveGame = (void (*)(MinecraftClient *, bool))
            dlsym(mcpelibhandle,
                  "_ZN15MinecraftClient9leaveGameEb"); //hooked - just pull whichever version MCPE uses
    bl_Minecraft_setLeaveGame = (void (*)(Minecraft *)) dlsym(mcpelibhandle,
                                                              "_ZN9Minecraft12setLeaveGameEv");
    ...
    mcpelauncher_hook(raknet_connect, (void *) &bl_RakNetInstance_connect_hook,
                      (void **) &bl_RakNetInstance_connect_real);
    ...
    bl_Block_Block = (void (*)(Block *, std::string const &, int, void *)) dlsym(
            RTLD_DEFAULT, "_ZN5BlockC1ERKSsiRK8Material");
    bl_BlockItem_BlockItem = (void (*)(Item *, std::string const &, short)) dobby_dlsym(
            mcpelibhandle, "_ZN9BlockItemC1ERKSsi");
...
#define CHECKVTABLE(actualfn) \
    ...
    bl_LiquidBlockStatic_LiquidBlockStatic = (void (*)(Block *, std::string const &, int, BlockID,
                                                       void *,
    ...
    bl_Item_setIcon = (void (*)(Item *, std::string const &, int)) dlsym(mcpelibhandle,
    ...
    bl_Tile_getTexture = (TextureUVCoordinateSet *(*)(Tile *, signed char, int)) dlsym(
            mcpelibhandle, "_ZN5Block10getTextureEai");
    bl_Recipes_getInstance = (Recipes *(*)()) dlsym(mcpelibhandle, "_ZN7Recipes11getInstanceEv");
    bl_Recipes_addShapedRecipe = (void (*)(Recipes *, std::vector<ItemInstance> const &,
                                           std::vector<std::string> const &,
    ...
    bl_FurnaceRecipes_getInstance = (FurnaceRecipes *(*)()) dlsym(mcpelibhandle,
                                                                  "_ZN14FurnaceRecipes11getInstanceEv");
    ...
    bl_Gui_showTipMessage = (void (*)(void *, const std::string &)) dlsym(RTLD_DEFAULT,
                                                                          "_ZN3Gui14showTipMessageERKSs");

    bl_Item_setMaxStackSize = (void (*)(Item *, unsigned char)) dlsym(mcpelibhandle,
    ...
    void **clientNetworkHandlerVtable =
            (void **) dlsym(mcpelibhandle, "_ZTV20ClientNetworkHandler") + 2;
    ...
    void **legacyClientNetworkHandlerVtable =
            (void **) dlsym(mcpelibhandle, "_ZTV26LegacyClientNetworkHandler") + 2;
    ...
    */
    ...
    void *addEntity = dlsym(mcpelibhandle,
                            "_ZN5Level9addEntityESt10unique_ptrI6EntitySt14default_deleteIS1_EE");
    mcpelauncher_hook(addEntity, (void *) &bl_Level_addEntity_hook,
                      (void **) &bl_Level_addEntity_real);
    void *mpAddEntity = dlsym(mcpelibhandle,
                              "_ZN16MultiPlayerLevel9addEntityESt10unique_ptrI6EntitySt14default_deleteIS1_EE");
    mcpelauncher_hook(mpAddEntity, (void *) &bl_MultiPlayerLevel_addEntity_hook,
                      (void **) &bl_MultiPlayerLevel_addEntity_real);
    void *addPlayer = dlsym(mcpelibhandle,
                            "_ZN5Level9addPlayerESt10unique_ptrI6PlayerSt14default_deleteIS1_EE");
    mcpelauncher_hook(addPlayer, (void *) &bl_Level_addPlayer_hook,
                      (void **) &bl_Level_addPlayer_real);
    void *onEntityRemoved = dlsym(mcpelibhandle, "_ZN5Level12removeEntityER6Entityb");
    mcpelauncher_hook(onEntityRemoved, (void *) &bl_Level_removeEntity_hook,
                      (void **) &bl_Level_removeEntity_real);

    // mcpelauncher_hook((void *) &Level::explode, (void *) &bl_Level_explode_hook,
    // 	(void **) &bl_Level_explode_real);XXX
    //
    // mcpelauncher_hook((void *) &BlockSource::fireBlockEvent, (void *) &bl_BlockSource_fireBlockEvent_hook,
    //                   (void **) &bl_BlockSource_fireBlockEvent_real);XXX

    //类函数
    void *explode = dlsym(mcpelibhandle, "_ZN5Level7explodeER11BlockSourceP6EntityRK4Vec3fb");
    mcpelauncher_hook(explode, (void *) &bl_Level_explode_hook,
                      (void **) &bl_Level_explode_real);
    void *fireBlockEvent = dlsym(mcpelibhandle, "_ZN11BlockSource14fireBlockEventEiiiii");
    mcpelauncher_hook(fireBlockEvent, (void *) &bl_BlockSource_fireBlockEvent_hook,
    ...
    bl_ReedBlock_getAABB = (AABB *(*)(Block *, BlockSource &, BlockPos const &, AABB &, int, bool,
                                      int))
...
#ifdef __arm__
    unsigned char *useControllerPtr = (unsigned char *) (
            ((uintptr_t) bl_MinecraftClient_useController) & ~1);
    ...
    bl_ItemEntity_ItemEntity = (void (*)(Entity *, BlockSource &, Vec3 const &,
                                         ItemInstance const &, int))
    ...
    */
    ...
    //                   (void **) &bl_Player_addExperience_real);XXX
    // mcpelauncher_hook((void *) &Player::addLevels, (void *) &bl_Player_addLevels_hook,
    //                   (void **) &bl_Player_addLevels_real);XXX

    //类函数
    void *addExperience = dlsym(mcpelibhandle, "_ZN6Player13addExperienceEi");
    mcpelauncher_hook(addExperience, (void *) &bl_Player_addExperience_hook,
                      (void **) &bl_Player_addExperience_real);
    void *addLevels = dlsym(mcpelibhandle, "_ZN6Player9addLevelsEi");
    mcpelauncher_hook(addLevels, (void *) &bl_Player_addLevels_hook,
    ...
    for (unsigned int i = 0;
         i < sizeof(listOfRenderersToPatchTextures) / sizeof(const char *); i++) {
    ...
    bl_SkeletonRenderer_render_real = (void *(*)(MobRenderer *, Entity &, Vec3 const &, float,
                                                 float))
