...
static void
(*bl_GameMode_useItemOn_real)(void *, Player *, ItemInstance *, TilePos *, signed char, Vec3 *);
static void
(*bl_SurvivalMode_useItemOn_real)(void *, Player *, ItemInstance *, TilePos *, signed char, Vec3 *);
static void
(*bl_MinecraftClient_onClientStartedLevel_real)(MinecraftClient *, std::unique_ptr<Level>,
                                                std::unique_ptr<LocalPlayer>);
...
void bl_GameMode_useItemOn_hook(void *gamemode, Player *player, ItemInstance *itemStack,
    ...
    jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "useItemOnCallback",
                                           "(IIIIIIII)V");

    env->CallStaticVoidMethod(bl_scriptmanager_class, mid, x, y, z, itemId, blockId, side,
                              itemDamage, blockDamage);
...
void bl_SurvivalMode_startDestroyBlock_hook(void *gamemode, Player *player, BlockPos blockPos,
                                            signed char side) {
    JNIEnv *env;
    preventDefaultStatus = FALSE;
    ...
    //Call back across JNI into the ScriptManager
    jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "startDestroyBlockCallback",
                                           "(IIII)V");

    env->CallStaticVoidMethod(bl_scriptmanager_class, mid, blockPos.x, blockPos.y, blockPos.z,
                              side);
    ...
    if (!preventDefaultStatus)
        bl_SurvivalMode_startDestroyBlock_real(gamemode, player, blockPos, side);
}

void bl_CreativeMode_startDestroyBlock_hook(void *gamemode, Player *player, BlockPos blockPos,
                                            signed char side) {
    JNIEnv *env;
    preventDefaultStatus = FALSE;
    ...
    if (!preventDefaultStatus)
        bl_CreativeMode_startDestroyBlock_real(gamemode, player, blockPos, side);
}

void *bl_GameMode_continueDestroyBlock_hook(void *gamemode, Player &player, BlockPos blockPos,
                                            signed char side) {
    JNIEnv *env;
    preventDefaultStatus = FALSE;
    ...
    //Call back across JNI into the ScriptManager
    jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "continueDestroyBlockCallback",
                                           "(IIIIF)V");
    ...
    if (!preventDefaultStatus)
        return bl_GameMode_continueDestroyBlock_real(gamemode, player, blockPos, side);
    return nullptr;
}
...
void bl_MinecraftClient_onClientStartedLevel_hook(MinecraftClient *minecraft,
                                                  ...
                                                  std::unique_ptr<LocalPlayer> localPlayerPtr) {
    JNIEnv *env;
    ...
    env->CallStaticVoidMethod(bl_scriptmanager_class, mid, (int) (level != NULL),
                              (jboolean) level->isRemote);
...
void *bl_MinecraftClient_startLocalServer_hook(MinecraftClient *minecraft, void *wDir, void *wName,
                                               void *levelSettings) {
    ...
    // Call back across JNI into the ScriptManager
    jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "selectLevelCallback",
...
void bl_GameMode_attack_hook(void *gamemode, Player *player, Entity *entity) {
    ...
    env->CallStaticVoidMethod(bl_scriptmanager_class, mid, player->getUniqueID(),
                              entity->getUniqueID());
...
void
bl_GameMode_destroyBlock_hook(void *gamemode, Player *player, BlockPos blockPos, signed char side) {
    JNIEnv *env;
    preventDefaultStatus = FALSE;
    bl_JavaVM->AttachCurrentThread(&env, NULL);

    //Call back across JNI into the ScriptManager
    jmethodID mid = env->GetStaticMethodID(bl_scriptmanager_class, "destroyBlockCallback",
                                           "(IIII)V");
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeAddItemChest
        (JNIEnv *env, jclass clazz, jint x, jint y, jint z, jint slot, jint id, jint damage,
         jint amount) {
    if (bl_level == NULL) return;
    ItemInstance *instance = bl_newItemInstance(id, amount, damage);

    ChestBlockEntity *te = static_cast<ChestBlockEntity *>(bl_localplayer->getRegion()->getBlockEntity(
            x, y, z));
    if (te == NULL) return;
...
JNIEXPORT jint JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeGetBrightness
        (JNIEnv *env, jclass clazz, jint x, jint y, jint z) {
    return bl_localplayer->getRegion()->getRawBrightness(x, y, z,
                                                         true).value; //all observed uses of getRawBrightness pass true
}
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeAddItemInventory
    ...
    Inventory *invPtr = *((Inventory **) (((uintptr_t) bl_localplayer) +
                                          PLAYER_INVENTORY_OFFSET)); //TODO fix this for 0.7.2
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeSetFov
    ...
    if (override) {
        memcpy((void *) (
                       (uintptr_t) bl_marauder_translation_function((void *) bl_LevelRenderer_getFov) &
                       ~1),
...
JNIEXPORT jint JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeGetSlotInventory
    ...
    if (bl_localplayer == NULL) return 0;
    //we grab the inventory instance from the player
    void *invPtr = *((void **) (((intptr_t) bl_localplayer) +
                                PLAYER_INVENTORY_OFFSET)); //TODO fix this for 0.7.2
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeSetInventorySlot
    ...
    if (bl_localplayer == NULL) return;
    //we grab the inventory instance from the player
    Inventory *invPtr = *((Inventory **) (((intptr_t) bl_localplayer) +
                                          PLAYER_INVENTORY_OFFSET)); //TODO Merge this into a macro
...
Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeSetEntityRenderType
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeAddItemFurnace
    ...
    FurnaceBlockEntity *tileEnt = static_cast<FurnaceBlockEntity *>(bl_localplayer->getRegion()->getBlockEntity(
            x, y, z));
...
static void populate_vtable_indexes(void *mcpelibhandle) {
    ...
    Dl_info info;
    if (dladdr(
            (void *) &Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeRequestFrameCallback,
            &info)) {
        int hash = 0;
...
void bl_prepatch_cside(void *_mcpelibhandle, JNIEnv *env, jclass clazz,
    ...
    bl_ItemInstance_getId = (int (*)(ItemInstance *)) dlsym(RTLD_DEFAULT,
                                                            "_ZNK12ItemInstance5getIdEv");
    bl_ItemInstance_setUserData = (void (*)(ItemInstance *, std::unique_ptr<CompoundTag>)) dlsym(
            mcpelibhandle,
    ...
    bl_ItemInstance_setId = (void (*)(ItemInstance *, int))
            dlsym(RTLD_DEFAULT,
                  "_ZN12ItemInstance8_setItemEi"); //note the name change: consistent naming
...
int bl_vtableIndex(void *si, const char *vtablename, const char *name) {
    ...
    Elf32_Sym *vtableSym = dobby_elfsym(si, vtablename);
    ...
    __android_log_print(ANDROID_LOG_ERROR, "BlockLauncher", "cannot find vtable entry %s in %s",
                        name, vtablename);
    return 0;
}
...
JNIEXPORT void JNICALL Java_net_zhuoweizhang_mcpelauncher_ScriptManager_nativeSetupHooks
    ...
    bl_GameMode_useItemOn_real = (void (*)(void *, Player *, ItemInstance *, TilePos *, signed char,
                                           Vec3 *))
    ...
    bl_SurvivalMode_useItemOn_real = (void (*)(void *, Player *, ItemInstance *, TilePos *,
                                               signed char, Vec3 *))
    ...
    void *selectLevel = dlsym(mcpelibhandle,
    ...
    mcpelauncher_hook(destroyBlock, (void *) &bl_GameMode_destroyBlock_hook,
                      (void **) &bl_GameMode_destroyBlock_real);
    ...
    mcpelauncher_hook(getFov, (void *) &bl_LevelRenderer_getFov_hook,
                      (void **) &bl_LevelRenderer_getFov_real);
    memcpy(getFovHooked, (void *) ((uintptr_t) getFov & ~1), sizeof(getFovHooked));
    // start off with original FOV
    memcpy((void *) ((uintptr_t) bl_marauder_translation_function(getFov) & ~1), getFovOriginal,
           sizeof(getFovOriginal));
    ...
    bl_LocalPlayer_hurtTo = (void (*)(Player *, int)) dlsym(RTLD_DEFAULT,
    ...
    bl_FillingContainer_getItem = (ItemInstance *(*)(void *, int)) dlsym(RTLD_DEFAULT,
                                                                         "_ZNK16FillingContainer7getItemEi");
    ...
    bl_Inventory_Inventory = (void (*)(void *, Player *)) dlsym(RTLD_DEFAULT,
                                                                "_ZN9InventoryC1EP6Player");
    ...
    bl_NinecraftApp_update_real = (void (*)(
            MinecraftClient *)) minecraftVtable[vtable_indexes.minecraft_update];
#if 0
    for (int i = 0; i < 40; i++) {
    ...
    bl_Inventory_getSelectedSlot = (int (*)(void *)) dlsym(mcpelibhandle,
                                                           "_ZNK9Inventory15getSelectedSlotEv");
    bl_Inventory_selectSlot = (void (*)(void *, int)) dlsym(mcpelibhandle,
                                                            "_ZN9Inventory10selectSlotEi");
    bl_AgableMob_getAge = (int (*)(Entity *)) dlsym(mcpelibhandle,
                                                    "_ZN9AgableMob6getAgeEv");

    bl_setuphooks_cppside(mcpelibhandle);
