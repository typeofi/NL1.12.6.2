...
static const uint32_t Elf32_MAGIC = 0x464C457f; // "ELF\0177"
...
static void load_ptrace_map_info_data(pid_t pid, map_info_t *mi) {
    if (mi->is_executable && mi->is_readable) {
        uint32_t Elf32_magic;
        if (try_get_word_ptrace(pid, mi->start, &Elf32_magic) && Elf32_magic == Elf32_MAGIC) {
...
void find_symbol_ptrace(const ptrace_context_t *context,
                        uintptr_t addr, const map_info_t **out_map_info,
